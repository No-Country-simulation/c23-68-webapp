...
const Ingresos = () => {
  ...
  useEffect(() => {
    const getElements = async () => {
      const data = await getTransactions("Ingreso");
      setData(data.data);
    };
    getElements();
  }, []);
  ...
  const filteredData = data
    ...
    .filter((item) =>
      selectedDate
        ? format(new Date(item.date), "DD/MM/YYYY") === selectedDate
        : true
    );
  ...
  return (
    <div className="flex flex-col min-h-screen pt-3 font-onest bg-gris3">
      <div className="flex items-center justify-between relative ml-[5%] my-9 w-[90%] max-w-[1400px]">
        ...
        <select
          className="w-[18%] ml-4 text-gris4 text-lg px-6 py-3 border rounded-lg"
