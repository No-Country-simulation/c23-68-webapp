...
const Ingresos = () => {
  ...
  const [data, setData] = useState([
    { tipo: 'Ingreso', monto: '$500', categoria: 'Sueldo', descripcion: 'Sin descripción', fecha: '13/01/2025' },
    { tipo: 'Ingreso', monto: '$900', categoria: 'Emprendim.', descripcion: 'Sin descripción', fecha: '14/01/2025' },
    { tipo: 'Ingreso', monto: '$900', categoria: 'Emprendim.', descripcion: 'Sin descripción', fecha: '14/01/2025' },
    { tipo: 'Ingreso', monto: '$900', categoria: 'Emprendim.', descripcion: 'Sin descripción', fecha: '14/01/2025' },
    { tipo: 'Ingreso', monto: '$900', categoria: 'Emprendim.', descripcion: 'Sin descripción', fecha: '14/01/2025' },
    { tipo: 'Ingreso', monto: '$900', categoria: 'Emprendim.', descripcion: 'Sin descripción', fecha: '14/01/2025' },
  ...
  const optionStyles = {
    ...
    BiFontFamily: 'Onest', // Tipo de fuente
  };

  return (
    <div className="font-onest bg-gris3 h-auto   pt-3">
      <div className="flex items-center justify-between relative ml-[25%] my-9 w-[70%] max-w-[1400px]">
        <div className="relative w-[60%] mr-4">
          <input
            ...
            className="text-gris4 text-lg p-2 pl-10 border rounded-lg w-full"
          />
          <svg
        ...
        <select className=" w-[18%] ml-4 text-gris4 text-lg px-6 py-3 border rounded-lg">
          <option style={optionStyles} value="">Fecha</option>
          <option style={optionStyles} value="opcion1">13/01/2025</option>
          <option style={optionStyles} value="opcion2">14/01/2025</option>
          <option style={optionStyles} value="opcion3">15/01/2025</option>
        </select>
      </div>

      <div className="flex items-center justify-between w-[95%] max-w-[1400px] mt-11">
        <h2 className=" ml-[27%] text-3xl text-gris">Historial</h2>
        <button className=" px-5 py-2 bg-verde font-onest text-white text-lg rounded-lg shadow-lg hover:bg-green-600">
          <svg 
            className="inline-block mr-2"
            width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M10 18.5C12.1217 18.5 14.1566 17.6571 15.6569 16.1569C17.1571 14.6566 18 12.6217 18 10.5C18 8.37827 17.1571 6.34344 15.6569 4.84315C14.1566 3.34285 12.1217 2.5 10 2.5C7.87827 2.5 5.84344 3.34285 4.34315 4.84315C2.84285 6.34344 2 8.37827 2 10.5C2 12.6217 2.84285 14.6566 4.34315 16.1569C5.84344 17.6571 7.87827 18.5 10 18.5ZM11 7.5C11 7.23478 10.8946 6.98043 10.7071 6.79289C10.5196 6.60536 10.2652 6.5 10 6.5C9.73478 6.5 9.48043 6.60536 9.29289 6.79289C9.10536 6.98043 9 7.23478 9 7.5V9.5H7C6.73478 9.5 6.48043 9.60536 6.29289 9.79289C6.10536 9.98043 6 10.2348 6 10.5C6 10.7652 6.10536 11.0196 6.29289 11.2071C6.48043 11.3946 6.73478 11.5 7 11.5H9V13.5C9 13.7652 9.10536 14.0196 9.29289 14.2071C9.48043 14.3946 9.73478 14.5 10 14.5C10.2652 14.5 10.5196 14.3946 10.7071 14.2071C10.8946 14.0196 11 13.7652 11 13.5V11.5H13C13.2652 11.5 13.5196 11.3946 13.7071 11.2071C13.8946 11.0196 14 10.7652 14 10.5C14 10.2348 13.8946 9.98043 13.7071 9.79289C13.5196 9.60536 13.2652 9.5 13 9.5H11V7.5Z" fill="white"/>
          </svg>
          Agregar
        </button>
      </div>

      <div className="bg-white shadow-lg rounded-3xl border-b-2 p-6 w-[70%] max-w-[1400px] my-10 ml-[25%]">
        <div className="w-full border-collapse bg-white rounded-xl">
          <table className="min-w-full">
            <thead>
              <tr className="bg-white">
                <th className="font-onest font-semibold text-negro  text-2xl p-2 pt-4">Tipo</th>
                <th className="font-onest font-semibold text-negro  text-2xl p-2 pt-4">Monto</th>
                <th className="font-onest font-semibold text-negro  text-2xl p-2 pt-4">Categoría</th>
                <th className="font-onest font-semibold text-negro  text-2xl p-2 pt-4">Descripción</th>
                <th className="font-onest font-semibold text-negro  text-2xl p-2 pt-4">Fecha</th>
              </tr>
            </thead>
            <tbody>
              {filteredData.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="font-onest font-normal text-negro text-lg p-2 text-center pb-4">{item.tipo}</td>
                  <td className="font-onest font-normal text-negro text-lg p-2 text-center pb-4">{item.monto}</td>
                  <td className="font-onest font-normal text-negro text-lg p-2 text-center pb-4">{item.categoria}</td>
                  <td className="font-onest font-normal text-negro text-lg p-2 text-center pb-4">{item.descripcion}</td>
                  <td className="font-onest font-normal text-negro text-lg p-2 text-center pb-4">{item.fecha}</td>
                ...
                <button className="bg-yellow-500 text-white px-4 py-2 rounded">
                  ...
                  <path d="M17.914 2.58606C17.5389 2.21112 17.0303 2.00049 16.5 2.00049C15.9697 2.00049 15.4611 2.21112 15.086 2.58606L7.5 10.1721V13.0001H10.328L17.914 5.41406C18.2889 5.03901 18.4996 4.53039 18.4996 4.00006C18.4996 3.46973 18.2889 2.96112 17.914 2.58606Z" fill="white"/>
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M2.5 6C2.5 5.46957 2.71071 4.96086 3.08579 4.58579C3.46086 4.21071 3.96957 4 4.5 4H8.5C8.76522 4 9.01957 4.10536 9.20711 4.29289C9.39464 4.48043 9.5 4.73478 9.5 5C9.5 5.26522 9.39464 5.51957 9.20711 5.70711C9.01957 5.89464 8.76522 6 8.5 6H4.5V16H14.5V12C14.5 11.7348 14.6054 11.4804 14.7929 11.2929C14.9804 11.1054 15.2348 11 15.5 11C15.7652 11 16.0196 11.1054 16.2071 11.2929C16.3946 11.4804 16.5 11.7348 16.5 12V16C16.5 16.5304 16.2893 17.0391 15.9142 17.4142C15.5391 17.7893 15.0304 18 14.5 18H4.5C3.96957 18 3.46086 17.7893 3.08579 17.4142C2.71071 17.0391 2.5 16.5304 2.5 16V6Z" fill="white"/>
                  </svg>
                  Editar
                </button>
      ...
      <div className="bg-white shadow-lg rounded-2xl border-b-2 p-4 w-[15%] max-w-[1400px] my-10 ml-[25%] font-onest text-negro text-lg font-normal">
              Gasto Total: $9,500
      </div>
